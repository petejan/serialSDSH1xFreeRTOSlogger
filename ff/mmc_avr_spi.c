...
#define CS_LOW()		(PORTB &= ~CS)	/* Set MMC_CS = low */
#define	CS_HIGH()		(PORTB |= CS)	/* Set MMC_CS = high */
...
void power_on (void)
{
	avrSerialPrint_P(PSTR("mmc_power_on\r\n"));
...
void power_off (void)
{
    ...
    avrSerialPrint_P(PSTR("mmc_power_off\r\n"));
}
...
{
	BYTE n, cmd, ty, ocr[4];
	avrSerialPrint_P(PSTR("mmc_init\r\n"));
